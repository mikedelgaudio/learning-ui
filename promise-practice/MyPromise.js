  ...
  #onFail(value) {
    queueMicrotask(() => {
      ...
      if (this.#catchCbs.length === 0) {
        // No catch provided to handle error
  ...
  then(thenCb, catchCb) {
    return new MyPromise((resolve, reject) => {
      this.#thenCbs.push((result) => {
        ...
        try {
          resolve(thenCb(result)); // Promise.then(return "hi").catch().then()
        } catch (e) {
          reject(e);
        }
      });
      ...
      this.#catchCbs.push((result) => {
        ...
        try {
          resolve(catchCb(result)); // Promise.then(return "hi").catch().then()
        } catch (e) {
          reject(e);
        }
      });
    ...
    return new MyPromise((resolve, reject) => {
      for (let i = 0; i < promises.length; i++) {
        ...
        promise
          ...
          .catch(reject); // if any fail reject
      }
    });
  }
